...
> **Reference**: This section defines retry limits and escalation paths to prevent infinite loops and ensure pipeline progress.

### Task-Level Retries

Each task gets a maximum of **3 retry attempts** before being marked as BLOCKED:
...
```javascript
// Retry tracking in state file
// retry_counts: { "T001": 2, "T003": 1 }
...
if (task.failed) {
  ...
  if (retryCount <= 3) {
    ...
    // Launch appropriate fix agent based on attempt number
    launchFixAgent(task, retryCount)
  } else {
    // Mark as BLOCKED
    state.blocked_tasks.push(task.id)
    state.progress.blocked++
    // Skip this task and all tasks that depend on it
...
4. Mark dependent tasks as `"skipped"` (they cannot run)
...
| Attempt | Strategy |
|---------|----------|
| 1st fix | Launch fix agent (debugger) |
| 2nd fix | Launch specialist agent (backend or frontend) |
| 3rd fix | Launch agent with broader context |
| After 3rd | **DEGRADED** → continue with warning |

```javascript
fixAttempts = 0
maxFixAttempts = 3

while (testsFailing && fixAttempts < maxFixAttempts) {
  fixAttempts++
  launchFixAgent(failures, fixAttempts)
  rerunTests()
}

if (testsFailing) {
  state.degraded_phases.push(currentPhase)
  state.quality_gates[currentPhase] = "degraded"
  // Continue to next phase - don't block the pipeline
}
```

### QA Cycle Limit

QA review cycles are limited to **2 iterations**:
...
All BLOCKED tasks and DEGRADED phases are reported in:
- The execution completion report
- The QA report (if QA phase runs)
- The state file (`blocked_tasks[]` and `degraded_phases[]`)
