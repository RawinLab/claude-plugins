...
> **Full details**: See `.claude/kbs/retry-policy.md` for the complete retry policy.

### Quick Reference

| Scope | Max Retries | On Exhaust |
|-------|-------------|------------|
| Task implementation | 3 per task | BLOCKED → skip task + dependents |
| Test phase fix loop | 3 per phase | DEGRADED → continue pipeline |
| Smoke test auto-fix | 2 | DEGRADED → continue pipeline |
| QA review cycles | 2 | DEGRADED → complete with warnings |
